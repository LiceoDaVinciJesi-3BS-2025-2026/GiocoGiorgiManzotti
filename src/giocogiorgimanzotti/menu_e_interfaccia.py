...
def disegna_menu(schermo, fonts, pos_mouse, difficolta):
    ...
    rett_pannelli = {
        'difficolta':  pygame.Rect(inizio_x,              centro_y - altezza_pannello // 2, larghezza_pannello, altezza_pannello),
        'controlli':   pygame.Rect(inizio_x + passo,      centro_y - altezza_pannello // 2, larghezza_pannello, altezza_pannello),
        'inizio':      pygame.Rect(inizio_x + passo * 2,  centro_y - altezza_pannello // 2, larghezza_pannello, altezza_pannello),
        'classifica':  pygame.Rect(inizio_x + passo * 3,  centro_y - altezza_pannello // 2, larghezza_pannello, altezza_pannello),
    }
    ...
    for i, (valore_diff, testo_diff, colore_diff) in enumerate(dati_diff):
        bottone = pygame.Rect(pannello.centerx - largh_btn_diff // 2, y_bottone + i * 65, largh_btn_diff, 52)
    ...
    # Bottone INIZIA
    bottone_inizio = pygame.Rect(pannello.centerx - largh_btn_inizio // 2, pannello.centery - 35, largh_btn_inizio, 70)
    ...
    schermo.blit(surf_sub, surf_sub.get_rect(center=(pannello.centerx, pannello.y + 62)))

    # Linea separatrice sotto il sottotitolo
    pygame.draw.line(schermo, (200, 200, 210),
                     (pannello.x + 15, pannello.y + 76),
                     (pannello.x + pannello.width - 15, pannello.y + 76), 1)

    # Carica e mostra i primi 5 giocatori
    classifica = ottieni_classifica_ordinata()
    colori_podio = [(255, 215, 0), (192, 192, 192), (205, 127, 50)]  # oro, argento, bronzo

    if not classifica:
        surf_vuota = fonts['piccolo'].render("Nessuna vittoria ancora!", True, (160, 160, 160))
        schermo.blit(surf_vuota, surf_vuota.get_rect(center=(pannello.centerx, pannello.centery + 20)))
    else:
        ...
        for i, (nome, vittorie) in enumerate(classifica[:5]):
            ...
            if i % 2 == 0:
                pygame.draw.rect(schermo, (210, 210, 220),
            ...
            schermo.blit(surf_vit, (pannello.x + pannello.width - surf_vit.get_width() - 18, y_riga))
