...
def _scegli_punto_random_in_piattaforma(piattaforma):
    ...
    x = random.randint(int(piattaforma['x'] + margine), int(piattaforma['x'] + piattaforma['larghezza'] - margine))
    y = random.randint(int(piattaforma['y'] + margine), int(piattaforma['y'] + piattaforma['altezza'] - margine))
    return x, y
...
def lottatore_aggiorna_ai(lottatore, lista_piattaforme, nome_colore_target, tutti_lottatori):
    ...
    def calcola_distanza_da_lottatore(piattaforma):
        ...
        return math.sqrt((centro_x - lottatore['x'])**2 + (centro_y - lottatore['y'])**2)
    ...
    if lottatore['timer_ai'] > lottatore['tempo_reazione_ai'] or lottatore['piattaforma_target'] is None:
        ...
        if piattaforme_valide:
            ...
            if random.random() < lottatore['qualita_decisioni_ai']:
                lottatore['piattaforma_target'] = min(piattaforme_valide, key=calcola_distanza_da_lottatore)
            else:
            ...
            lottatore['target_x'], lottatore['target_y'] = _scegli_punto_random_in_piattaforma(
                lottatore['piattaforma_target']
            )
    ...
    if lottatore['piattaforma_target'] and lottatore['piattaforma_target']['attiva']:
        ...
        tx = lottatore['target_x'] if lottatore['target_x'] else piattaforma_ottieni_centro(lottatore['piattaforma_target'])[0]
        ty = lottatore['target_y'] if lottatore['target_y'] else piattaforma_ottieni_centro(lottatore['piattaforma_target'])[1]
...
def lottatore_disegna(schermo, lottatore):
    ...
    pygame.draw.circle(schermo, tuple(max(0, c - 40) for c in lottatore['colore_corpo']), pos, raggio_corpo)
    pygame.draw.circle(schermo, lottatore['colore_corpo'], (pos[0] - 3, pos[1] - 3), raggio_corpo - 2)
    pygame.draw.circle(schermo, tuple(min(255, c + 30) for c in lottatore['colore_corpo']), (pos[0] - 5, pos[1] - 5), raggio_corpo - 8)
    pygame.draw.circle(schermo, (0, 0, 0), pos, raggio_corpo, 3)

    # muscoli pettorali
    offset_muscoli = 8
    pygame.draw.circle(schermo, tuple(max(0, c - 20) for c in lottatore['colore_corpo']), (pos[0] - offset_muscoli, pos[1] - 5), 10)
    pygame.draw.circle(schermo, tuple(max(0, c - 20) for c in lottatore['colore_corpo']), (pos[0] + offset_muscoli, pos[1] - 5), 10)

    # addominali
    pygame.draw.line(schermo, tuple(max(0, c - 50) for c in lottatore['colore_corpo']), (pos[0] - 6, pos[1] + 5), (pos[0] - 3, pos[1] + 12), 2)
    pygame.draw.line(schermo, tuple(max(0, c - 50) for c in lottatore['colore_corpo']), (pos[0] + 3, pos[1] + 5), (pos[0] + 6, pos[1] + 12), 2)

    # cintura con fibbia dorata
    pygame.draw.rect(schermo, (40, 40, 40), (pos[0] - raggio_corpo, pos[1] - 2, raggio_corpo * 2, 8))
    pygame.draw.rect(schermo, (70, 70, 70), (pos[0] - raggio_corpo, pos[1] - 2, raggio_corpo * 2, 3))
    ...
    if lottatore['attaccando']:
        ...
        pygame.draw.line(schermo, (0, 0, 0), (pos[0] - offset_occhi - 3, occhi_y), (pos[0] - offset_occhi + 3, occhi_y - 2), 3)
        pygame.draw.line(schermo, (0, 0, 0), (pos[0] + offset_occhi - 3, occhi_y - 2), (pos[0] + offset_occhi + 3, occhi_y), 3)
    else:
        # occhi aperti con iride e pupilla
        pygame.draw.ellipse(schermo, (255, 255, 255), (pos[0] - offset_occhi - 4, occhi_y - 3, 8, 6))
        pygame.draw.ellipse(schermo, (255, 255, 255), (pos[0] + offset_occhi - 4, occhi_y - 3, 8, 6))
    ...
    # sopracciglia
    pygame.draw.line(schermo, (40, 30, 20), (pos[0] - offset_occhi - 5, occhi_y - 6), (pos[0] - offset_occhi + 3, occhi_y - 7), 2)
    pygame.draw.line(schermo, (40, 30, 20), (pos[0] + offset_occhi - 3, occhi_y - 7), (pos[0] + offset_occhi + 5, occhi_y - 6), 2)
    ...
    if lottatore['attaccando']:
        pygame.draw.arc(schermo, (100, 50, 50), (pos[0] - 6, testa_y + 10, 12, 8), 0, math.pi, 2)
    else:
        pygame.draw.arc(schermo, (150, 80, 80), (pos[0] - 7, testa_y + 8, 14, 10), math.pi, 2 * math.pi, 2)
